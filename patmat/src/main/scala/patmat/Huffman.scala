...
trait Huffman extends HuffmanInterface:
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    // assumption: `table` must contain `char`
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def addBitToEachElementOfCodeTable(table: CodeTable, bit: Int): CodeTable = table match
      case Nil => Nil
      case (char, bitsList) :: tail =>
        (char, bit +: bitsList) :: addBitToEachElementOfCodeTable(tail, bit)

    val left = addBitToEachElementOfCodeTable(a, 0)
    val right = addBitToEachElementOfCodeTable(b, 1)
    left ++ right
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def traverse(codeTable: CodeTable, text: List[Char], decodedBits: List[Bit]): List[Bit] =
      text match {
        case Nil => decodedBits
        case char :: tail =>
          val charToBits = codeBits(codeTable)(char)
          val newDecodedBits = decodedBits ++ charToBits
          traverse(codeTable, tail, newDecodedBits)
      }
    traverse(codeTable, text, Nil)
  }
